...
var Registry = []Switch{
	...
	{"FLOWORK_APPROVAL_MODE", "Mode approval aksi destruktif", "Gerbang approval interaktif (F-B, ala Claude Code). 'default' = aksi DESTRUKTIF (shell yang mengubah, termasuk git push/commit) masuk antrian approval owner dulu — read-only & edit file workspace tetap jalan. 'plan' = SEMUA aksi yang mengubah butuh approval (read-only aja yang lolos). 'bypass' = tanpa gerbang ekstra (perilaku lama; protector/caps/ARM tetap). 'acceptEdits' = alias default. Putusin antrian: GUI atau POST /api/agents/protector/approve_pending?id=<agent>&queue_id=<n> (approved berlaku 1 jam per tool+args). Per-agent bisa diperketat via config agent `approval_mode`='plan'.", "string", "default", "Security / Approval"},
