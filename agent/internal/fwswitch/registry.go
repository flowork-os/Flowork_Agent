...
var Registry = []Switch{
	...
	{"FLOWORK_REFLECT", "Mode Refleksi (ngelamun idle)", "OFF (default) = mati. ON = pas PC IDLE & papan kerja KOSONG, mr-flow 'ngelamun terstruktur': recall memori/pengalaman lama (brain_search/graph_recall → amplitude naik → LOLOS dari decay brain_dream) + simpan wawasan baru (brain_add → simpul Kebijaksanaan permanen). Bikin ingatan Mr.Dev ga memudar, malah matang jadi kebijaksanaan (Warisan Pikiran). WAJIB sadar konteks/nuansa memori biar maknanya ga ilang. Seed rule idle 'reflect-idle' (cooldown 90m, threshold 55). Ide dari mr-flow sendiri. Otonom + model lokal → opt-in.", "bool", "false", "Autonomy / Orchestration"},
