...
orbDropZone.addEventListener('drop', (e) => {
  ...
  // 自動送出：分析這些檔案
  chatInput.value = chatInput.value || 'analyze these files';
  handleChatSend();
});
...
async function handleChatSend() {
  ...
  // ── Flowork: chat SINKRON lewat /api/chat/send (target mr-flow, LLM lokal). ──
  // Gak ada SSE/stream (Flowork balas {reply} sekali jalan). File upload di-drop v1.
  try {
    const res = await fetch('/api/chat/send', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ session_id: window.__floworkSession || '', text: msg }),
    });
    const result = await res.json().catch(() => ({}));
    const reply = (result && (result.reply || result.error)) || '';
    if (!res.ok || !reply) {
      addChatLine(`ERROR: ${(result && result.error) || 'no reply dari mr-flow'}`, 'system-line');
    } else {
      window.dispatchEvent(new CustomEvent('agent-state', { detail: 'responding' }));
      addChatLine(reply, 'jin-line');
    }
  } catch (err) {
...
export function initChat() {
  ...
  if (chatInput) {
    ...
    chatInput.addEventListener('paste', (e) => {
      ...
      updateAttachBtn(pendingFiles.length);
      // 顯示通知
      addChatLine(`📋 ${files.length} image(s) pasted — type a message & send`, 'system-line');
    });
  }
  ...
  // ── Flowork: bikin sesi chat ke mr-flow (LLM lokal) sekali di awal. ──
  // Gak ada history/SSE (Flowork chat sinkron). session_id dipakai handleChatSend.
  fetch('/api/chat/sessions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ mode: 'group', target_id: 'mr-flow' }),
  })
    .then(r => r.json())
    .then(d => {
      window.__floworkSession = ((d && (d.session || d)).id) || '';
      addChatLine('FLOWORK LINK ONLINE — mr-flow (local) ✓', 'system-line');
    })
    .catch(() => addChatLine('FLOWORK LINK FAILED — cek login/host', 'system-line'));
