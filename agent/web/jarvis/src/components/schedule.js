...
function formatSchedule(schedule) {
  ...
  if (schedule.kind === 'cron') {
    ...
    if (parts.length === 5) {
      ...
      // every 某天: 0 9 * * 1 → every 一 09:00
      if (dom === '*' && mon === '*' && /^\d$/.test(dow)) {
        return `every ${dowNames[parseInt(dow)]} ${hour.padStart(2, '0')}:${min.padStart(2, '0')}`;
      }
    }
...
function formatHeartbeat(every) {
  if (!every) return 'every 30 min';
  const match = every.match(/^(\d+)(ms|s|m|h)$/i);
  if (!match) return every;
  const val = parseInt(match[1]);
  const unit = match[2].toLowerCase();
  if (unit === 'h') return `every ${val}h`;
  if (unit === 'm') return `every ${val}min`;
  if (unit === 's') return `every ${val}s`;
  return every;
}
      ...
      <div class="sched-items">
        ${heartbeat ? `
          <div class="sched-item">
            ...
            ${heartbeat.content ? `
              <div class="sched-hb-toggle" data-action="expand-hb">▸ view content</div>
  ...
  container.querySelectorAll('[data-action="expand-hb"]').forEach(toggle => {
    toggle.addEventListener('click', () => {
      ...
      if (content.classList.contains('hidden')) {
        ...
        toggle.textContent = '▸ view content';
      }
    });
  });
}
