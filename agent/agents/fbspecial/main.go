...
// (jaringan/timeout/5xx/429/408), TRANSPARAN ke loop utama (loop ga lihat retry). Fatal (4xx selain
// 429/408) → balik error langsung, NO retry. Akar fix "router error → auto-continue #8": blip sesaat
// di-recover di sini, BUKAN dianggap "task belum kelar". Prinsip Claude (withRetry.ts). Switch:
// FLOWORK_ROUTER_RETRY (default 5; 0/1 = matiin retry).
func callRouterRetry(reqJSON []byte) (*httpResp, error) {
	...
	for att := 1; att <= maxAtt; att++ {
		...
		if err == nil && resp != nil && resp.Status < 400 {
			return resp, nil // sukses
		}
		// Klasifikasi: transient = layak retry; fatal = stop (retry sia-sia).
		...
		// Backoff exponential + jitter (base 500ms, cap 30s). Jitter dari clock (no rand dep).
		d := 500 << uint(att-1) // 500,1000,2000,4000,8000...
		if d > 30000 {
			d = 30000
		}
...
// atoiSafe — parse int sederhana (TinyGo/wasip1-safe, no strconv churn). Balik -1 kalau invalid.
...
func callLLM(cfg agentConfig, userText string) string {
	...
	for iter := 0; iter < maxToolIters; iter++ {
		...
		if err != nil || resp == nil {
			// Transient abis retry / fatal → LAPOR JUJUR (doktrin resilience), BUKAN "router error" telanjang.
