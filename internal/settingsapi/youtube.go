...
//
// Audit 2026-06-07: guarded the refresh-error message slice — a revoked/expired
// token returns a SHORT body (e.g. {"error":"invalid_grant"}) and the old bare
// [:120] would panic with slice-bounds-out-of-range.
//
...
func (a *API) ytAccessToken() (string, error) {
	...
	if out.AccessToken == "" {
		// Guard the slice: a refresh error (e.g. {"error":"invalid_grant"} on a
		// revoked/expired token) is usually SHORTER than 120 chars, and a bare
		// [:120] would panic with slice-bounds-out-of-range.
		msg := strings.TrimSpace(out.Error + " " + string(b))
		if len(msg) > 120 {
			msg = msg[:120]
		}
		return "", fmt.Errorf("refresh gagal: %s", msg)
	}
