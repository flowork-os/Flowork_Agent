...
func init() {
	...
	tools.Register(&walletAddressAddTool{})
}
...
// =============================================================================
// 8. wallet_address_add
// =============================================================================

type walletAddressAddTool struct{}

func (walletAddressAddTool) Name() string       { return "wallet_address_add" }
func (walletAddressAddTool) Capability() string { return "state:write" }
func (walletAddressAddTool) Schema() tools.Schema {
	return tools.Schema{
		Description: "Add wallet address untuk tracking. chain_id 1=ethereum, 137=polygon, 56=bsc. Label optional.",
		Params: []tools.Param{
			{Name: "address", Type: tools.ParamString, Description: "Wallet address (0x...)", Required: true},
			{Name: "chain_id", Type: tools.ParamInt, Description: "EIP-155 chain ID (1=eth, 137=polygon, 56=bsc, default 1)", Required: false},
			{Name: "label", Type: tools.ParamString, Description: "Friendly label", Required: false},
		},
		Returns: "{ok}",
	}
}

func (walletAddressAddTool) Run(ctx context.Context, args map[string]any) (tools.Result, error) {
	store, ok := tools.FromStore(ctx)
	if !ok || store == nil {
		return tools.Result{}, fmt.Errorf("agent store not available")
	}
	...
	chainID := 1
	if n, ok := args["chain_id"].(float64); ok && n > 0 {
		chainID = int(n)
	}
	if address == "" {
		return tools.Result{}, fmt.Errorf("address required")
	}
	if err := store.AddWalletAddress(chainID, address, label); err != nil {
		return tools.Result{}, fmt.Errorf("add address: %w", err)
	}
	return tools.Result{
		Output: map[string]any{"ok": true, "address": address, "chain_id": chainID},
	}, nil
}
