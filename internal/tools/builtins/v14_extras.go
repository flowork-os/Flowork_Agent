...
func init() {
	...
	tools.Register(&walletAddressRemoveTool{})
...
// =============================================================================
// 2. wallet_address_remove
// =============================================================================

type walletAddressRemoveTool struct{}

func (walletAddressRemoveTool) Name() string       { return "wallet_address_remove" }
func (walletAddressRemoveTool) Capability() string { return "state:write" }
func (walletAddressRemoveTool) Schema() tools.Schema {
	return tools.Schema{
		Description: "Remove wallet address by chain_id + address.",
		Params: []tools.Param{
			{Name: "address", Type: tools.ParamString, Description: "Wallet address", Required: true},
			{Name: "chain_id", Type: tools.ParamInt, Description: "EIP-155 chain ID", Required: true},
		},
		Returns: "{ok}",
	}
}

func (walletAddressRemoveTool) Run(ctx context.Context, args map[string]any) (tools.Result, error) {
	store, ok := tools.FromStore(ctx)
	if !ok || store == nil {
		return tools.Result{}, fmt.Errorf("agent store not available")
	}
	address, _ := args["address"].(string)
	chainIDf, _ := args["chain_id"].(float64)
	if address == "" || chainIDf <= 0 {
		return tools.Result{}, fmt.Errorf("address + chain_id required")
	}
	if err := store.DeleteWalletAddress(int(chainIDf), address); err != nil {
		return tools.Result{}, fmt.Errorf("delete: %w", err)
	}
	return tools.Result{
