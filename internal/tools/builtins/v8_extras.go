...
func init() {
	...
	tools.Register(&walletAlertListTool{})
	tools.Register(&walletAlertsFiredListTool{})
...
// =============================================================================
// 4. wallet_alert_list — list wallet alert config
// =============================================================================

type walletAlertListTool struct{}

func (walletAlertListTool) Name() string       { return "wallet_alert_list" }
func (walletAlertListTool) Capability() string { return "state:read" }
func (walletAlertListTool) Schema() tools.Schema {
	return tools.Schema{
		Description: "List configured wallet alerts (threshold + channel + cooldown).",
		Params:      []tools.Param{},
		Returns:     "{count, alerts[]}",
	}
}

func (walletAlertListTool) Run(ctx context.Context, args map[string]any) (tools.Result, error) {
	store, ok := tools.FromStore(ctx)
	if !ok || store == nil {
		return tools.Result{}, fmt.Errorf("agent store not available")
	}
	items, err := store.ListWalletAlerts()
	if err != nil {
		return tools.Result{}, fmt.Errorf("list wallet alerts: %w", err)
	}
	return tools.Result{
...
// =============================================================================
// 5. wallet_alerts_fired_list — list alerts yang sudah trigger
// =============================================================================

type walletAlertsFiredListTool struct{}

func (walletAlertsFiredListTool) Name() string       { return "wallet_alerts_fired_list" }
func (walletAlertsFiredListTool) Capability() string { return "state:read" }
func (walletAlertsFiredListTool) Schema() tools.Schema {
	return tools.Schema{
		Description: "List wallet alerts yang sudah pernah fire. History anti-spam (1h cooldown default).",
		Params: []tools.Param{
...
func (walletAlertsFiredListTool) Run(ctx context.Context, args map[string]any) (tools.Result, error) {
	store, ok := tools.FromStore(ctx)
	if !ok || store == nil {
		return tools.Result{}, fmt.Errorf("agent store not available")
	}
	...
	items, err := store.ListWalletAlertsFired(limit)
	if err != nil {
		return tools.Result{}, fmt.Errorf("list fired: %w", err)
	}
	return tools.Result{
