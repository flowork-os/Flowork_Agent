...
func init() {
	tools.Register(&walletBalanceTool{})
...
// =============================================================================
// 1. wallet_balance — query wallet addresses + last snapshot
// =============================================================================

type walletBalanceTool struct{}

func (walletBalanceTool) Name() string       { return "wallet_balance" }
func (walletBalanceTool) Capability() string { return "state:read" }
func (walletBalanceTool) Schema() tools.Schema {
	return tools.Schema{
		Description: "Query wallet addresses + latest balance snapshot per agent. Self-introspection.",
		Params:      []tools.Param{},
		Returns:     "{count, addresses[]}",
	}
}

func (walletBalanceTool) Run(ctx context.Context, args map[string]any) (tools.Result, error) {
	...
	if err != nil {
		return tools.Result{}, fmt.Errorf("list wallet addresses: %w", err)
	}
	return tools.Result{
		Output: map[string]any{"count": len(addrs), "addresses": addrs},
	}, nil
}
