...
func init() {
	...
	tools.Register(&walletSnapshotsTool{})
...
// =============================================================================
// 2. wallet_snapshots — list wallet snapshots
// =============================================================================

type walletSnapshotsTool struct{}

func (walletSnapshotsTool) Name() string       { return "wallet_snapshots" }
func (walletSnapshotsTool) Capability() string { return "state:read" }
func (walletSnapshotsTool) Schema() tools.Schema {
	return tools.Schema{
		Description: "List historical wallet balance snapshots untuk agent. Default limit 20.",
		Params: []tools.Param{
			{Name: "limit", Type: tools.ParamInt, Description: "Max (default 20, max 100)", Required: false},
		},
		Returns: "{count, snapshots[]}",
	}
}

func (walletSnapshotsTool) Run(ctx context.Context, args map[string]any) (tools.Result, error) {
	store, ok := tools.FromStore(ctx)
	if !ok || store == nil {
		return tools.Result{}, fmt.Errorf("agent store not available")
	}
	...
	items, err := store.ListWalletSnapshots(limit)
	if err != nil {
		return tools.Result{}, fmt.Errorf("list wallet snapshots: %w", err)
	}
	return tools.Result{
		Output: map[string]any{"count": len(items), "snapshots": items},
	}, nil
}
