...
```
Hit **Install + enable**. Flowork starts that server and its tools become available to all agents
(untick per-agent later, in each agent's Tools). Each MCP card shows its tools + Enable/Disable +
Uninstall.
...
`.fwpack` in. For MCP you build nothing — just paste a server's config (client) or point an external
client at Flowork's MCP server.
