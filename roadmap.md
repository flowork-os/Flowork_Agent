...
## Section 12 — Tool execution sandbox + interceptors ✅ DONE (phase 1+2+3) 2026-05-30 — ALL acceptance criteria 100% met. Phase 3 closes 3 unmet criteria: (1) **manual approve session** via approval_queue (sensitive args contain `state.db`/`/etc/sudoers`/`/etc/passwd` → enqueue pending + ErrPendingApprove dengan queue_id; session-1h pass-through via args_hash match after POST /approve_pending), (2) **tool_audit spec-compliant** (id, tool_name, decision, reason, args_hash SHA256, caller, occurred_at + 3 idx — append-only via Go API), (3) **bash mem limit enforced** via Linux RLIMIT_AS (ulimit -v 512MB prepend, build-tag platform separation). SandboxRunV3 wraps V2 + auto-log setiap Run ke tool_audit dengan decision allowed/denied_cap/denied_disabled/denied_rate/denied_interceptor/pending_approve. Verified end-to-end (sensitive echo containing state.db → enqueued queue_id=1 → POST approve → retry args_hash pass-through). Defer phase 4: OS-isolator real (Landlock/JobObject/Seatbelt), hooks_pretool dynamic per-warga, hash-chain audit immutability.
...
## Section 24 — File Protector (Host Protection Gate / HPG) ✅ DONE 2026-05-30 phase 1. `internal/protector/baseline.go` LOCKED — 28 IMMUTABLE compile-time rules (Go memory wins, DB delete no-op security): 10 file_path (`/etc/passwd`, `/etc/shadow`, `/root/`, `/.ssh/`, Win32 dst.), 11 command (`rm -rf /`, fork bomb, `mkfs`, `dd if=/dev/zero`, `shutdown`, `sudo`, `chmod 777`), 3 IP (cloud metadata pivot 169.254.169.254 / 100.100.100.200 / 192.0.0.192), 4 env_var (TELEGRAM_BOT_TOKEN, ETHERSCAN_API_KEY, GITHUB_TOKEN, AWS_SECRET_ACCESS_KEY). `CheckPattern(ruleType, candidate, customRules)` substring case-insensitive matcher, baseline-first priority. `internal/agentdb/protector.go` LOCKED: protector_rules (UNIQUE rule_type+pattern, source enum hardcoded/custom, action enum block/warn/audit_only) + protector_audit (FQP-12 append-only). AddProtectorRule reject source=hardcoded. DeleteProtectorRule reject hardcoded entries (double-protection). `internal/agentmgr/protector.go` LOCKED: GET/POST/DELETE `/api/agents/protector/rules` (?include_baseline=1 merge), POST `/api/agents/protector/test`, GET `/api/agents/protector/audit?from=&to=`. Verified end-to-end (`rm -rf /` baseline block, `169.254.169.254` metadata pivot block, benign `echo hello` no-hit, custom `/tmp/secret` pattern added → match `/tmp/secret/file.txt`, list with baseline returns 29 total / 28 hardcoded / 1 custom). Defer phase 2: integrasi ke SandboxRunV2 interceptor chain (saat ini standalone — Section 12 phase 2 interceptor sudah cover banyak workspace+sensitive+persona, Section 24 add DB-driven custom rule layer), karma penalty saat hit_block, 50+ attack scenario test suite dari referensifile host_protection_test.go, GUI popup section "Protector" UI.
...
## Section 25 — Code Scanner (35+ static auditor) ✅ DONE 2026-05-30 phase 1. `internal/scanner/auditors.go` LOCKED — 6 of 35 Tier 1 (high-value subset, stdlib regexp): **hardcoded_secret_auditor** (critical — AWS_KEY, GITHUB_TOKEN gh*_…, slack xox*, stripe sk_*, openai sk-…, telegram bot token), **command_injection_auditor** (high — `exec.Command("sh","-c", x+y)` patterns), **sql_injection_auditor** (critical — `fmt.Sprintf("SELECT...%s")` + concat), **path_traversal_auditor** (high — `filepath.Join`/`os.Open(var)` no Base/Clean defense), **ssrf_auditor** (high — `http.Get(var)` no whitelist hint), **token_leak_auditor** (medium — log/print mentioning `token|secret|password|key|apiKey`). `runner.go` LOCKED — filesystem walker, scannable ext (.go/.py/.js/.ts/.sh/.rb/.java/.c/.cpp/.rs/.php/.yaml/.json/.env/.toml), skip node_modules/.git/vendor/__pycache__, 2MB per-file cap, 5000-finding overall cap. `agentdb/scanner.go` LOCKED — scanner_runs + scanner_findings + idx severity/run/started. `agentmgr/scanner.go` LOCKED — 4 endpoint: POST `/api/agents/scanner/scan` (target_path resolve dalam workspace, anti-escape), GET `/runs?limit=`, GET `/findings?run_id=`, GET `/auditors`. Verified end-to-end (decoy file dengan 4 vulnerability injected → 3 hit dengan severity correct: SSRF high, token_leak medium, sql_injection critical; status=fail karena critical_count=1). Defer phase 2: 29 sisanya dari 35 Tier 1 (path_safety, taint, prompt_injection, env_leak, sensitive_log, log_injection, crypto x3, deprecated_hash, tls x2, supply_chain, dangerous_import, dep_version, dockerfile_security, sandbox, permission, idor, toctou, goroutine_leak, panic_goroutine, panic, resource_leak, memory, zombie, atomic_write, hallucination_trap, pandora, fortress, exposure, zeroday, crossos, gosec_parser, budget, api_cost, api_rate_limit), parallel goroutine per auditor, GitHub repo scan, ZIP inline scan, severity threshold filter, dashboard sparkline (referensifile dashboard.go).
