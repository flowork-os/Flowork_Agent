...
func pluginUninstallHandler(store *floworkdb.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// hapus DB (crew + kategori)
		if err := store.DeleteCategory(catID); err != nil {
			tfWriteJSON(w, http.StatusInternalServerError, map[string]any{"error": "delete category: " + err.Error()})
			return
		}
		// hapus agent dir yang GA dipake kategori lain → watcher ChangeRemoved → unload
...
func pluginExportHandler(store *floworkdb.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		agentIDs := []string{}
		if cat.Synthesizer != "" {
			man.Crew = append(man.Crew, pluginCrewMember{AgentID: cat.Synthesizer, RoleLabel: "synthesizer", Kind: "synth"})
			agentIDs = append(agentIDs, cat.Synthesizer)
		}
		for _, c := range cat.Crew {
			man.Crew = append(man.Crew, pluginCrewMember{AgentID: c.AgentID, RoleLabel: c.RoleLabel, Kind: "worker"})
